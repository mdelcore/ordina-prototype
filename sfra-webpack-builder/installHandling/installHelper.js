...
function install(cartridgeFolder, packageInstallMode) {
    ...
    if (hasPackageJson) {
        childProcess.execSync(packageInstallMode + ' install', { cwd: cartridgeFolder, env: process.env, stdio: 'inherit', windowsHide: true });
    }
    if (parentHasPackageJson) {
        childProcess.execSync(packageInstallMode + ' install', { cwd: parentFolder, env: process.env, stdio: 'inherit', windowsHide: true });
    }
}
/**
